...
İster yalnızca Twitch'te yayın yapın, ister Twitch ve Kick üzerinde aynı anda çoklu yayın gerçekleştirin; Yayın Yöneticisi farklı tarayıcı sekmeleri veya platform panelleri arasında geçiş yapmanıza gerek kalmadan tüm yayınınızı senkronize tutar.
...
- **Hedef Platform Seçimi**: Değişiklikleri yalnızca **Twitch**, yalnızca **Kick** veya **Her İki Platform** için aynı anda uygulayın.
...
- **Twitch Etiketleri**: Twitch yayın etiketlerini pencere üzerinden ekleyin veya kaldırın.
- **Çapraz Platform Senkronizasyonu**:
  - Başlık veya kategori Twitch ve Kick arasında farklı olduğunda anında uyarı görüntüler (*"Başlık veya kategori platformlar arasında farklı"*).
  - Tek tıkla kopyalama araçları: **Kick'ten kopyala**, **Kick'ten başlığı kopyala**, **Kick'ten kategoriyi kopyala** veya **Kick'ten etiketleri kopyala**.
  - Tek tıkla eşitleme butonları: **Twitch başlığını kullan** / **Kick başlığını kullan**, **Twitch kategorisini kullan** / **Kick kategorisini kullan**.

### 2. Çoklu Platform Canlı Sohbet

Topluluğunuzu bir araya getiren düşük gecikmeli canlı sohbet akışı:

- **Birleşik Sohbet Akışı**: Twitch ve Kick mesajlarını belirgin platform ve kullanıcı rozetleriyle yan yana okuyun.
- **Platforma Özel Mesaj Gönderme**: Doğrudan Twitch veya Kick sohbetine mesaj ya da duyuru göndermek için hedef platformu seçin.
...
- **Mesajı Sil**: Uygunsuz mesajları Twitch sohbetinden anında kaldırın.
...
- **İzin Yardımcısı**: Hesabınızda gerekli Twitch moderatör izinleri eksikse tek tıkla güncelleme bağlantısı sunar (**İzinleri Ver**).
...
- **Platform Adını Oku**: Çoklu platform yayınlarında mesajın hangi platformdan geldiğini otomatik olarak seslendirir (örn. *"Kick üzerinde..."*).
...
- **Twitch Etkinlikleri**:
  ...
  - Bit / Cheer gönderimleri (`@{user} {bits} bit gönderdi`)
  - Kanal Puanı ödül kullanımları (`@{user} "{reward}" ödülünü kullandı`)
...
1. **Panel → Twitch → `<kanal>` → Ayarlar → Moderatörler** (`/dashboard/twitch/<kanal-id>/settings`) sayfasına gidin.
