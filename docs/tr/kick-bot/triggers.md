...
description: "Senchabot Kick botu için kanal mesaj tetikleyicileri: eşleşme tipleri, izinler, yanıt eylemleri (moderasyon dahil), değişkenler ve bekleme süreleri."
---

# Kick Botu — Tetikleyiciler <Badge type="warning" text="NEW"/>
...
yanıttan sohbet zaman aşımına, mesaj silmeye veya kalıcı bir yasağa kadar her
şey olabilir.
...
| `timeout_user`         | tetikleyen kullanıcıya Kick sohbet zaman aşımı uygular (süre, platformun moderatör zaman aşımları için varsayılanına uyar) |
| `ban_user`             | kullanıcıyı kanaldan kalıcı olarak yasaklar                            |

Moderasyon eylemleri (`timeout_user`, `ban_user`), botun Kick kanalınızda
moderatör ayrıcalıklarına sahip olmasını gerektirir. Kick'in sohbet API'si
şu anda botlara mesaj-silme uç noktası sunmadığından `delete_message` Kick'te
desteklenmez.
...
(`/dashboard/kick/<kanal-id>/moderation`) adresinde bulunur. Bu sayfa iki
bölüm sunar:
...
- **Moderasyon Hazır Ayarları** — açıp kapatabileceğiniz hazır kurallar.
  ...
  davranır ve suçlu kullanıcı için `timeout_user` eylemini tetikler
  (desteklenen yerlerde tekrarlayan suçlular için `ban_user` da çalışır).
...
bir kelimeyi içeren her mesaj bot tarafından otomatik olarak işlenir (zaman
aşımı ve/veya yasaklama).
...
::: bilgi Kick'teki eylem farkları
Kick'te moderasyon hazır ayarları ve engellenen-kelimeler işleyicisi, Kick'in
sohbet API'si botlara mesaj-silme uç noktası sunmadığından, suçlu mesajı
silmek yerine suçlu kullanıcıyı zaman aşımına uğratır (veya yasaklar).
:::
