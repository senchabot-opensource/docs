---
title: Twitch Bot — Stream Manager
description: "Senchabot Stream Manager for Twitch: Unified multiplatform live chat, stream metadata editor, activity feed, advanced Text-To-Speech (TTS), and moderation controls."
---

# Twitch Bot — Stream Manager <Badge type="warning" text="NEW"/>
...
Whether you stream exclusively on Twitch or multistream across Twitch and Kick, Stream Manager keeps everything in sync without needing to switch between different browser tabs or creator dashboards.
...
- **Target Platform Selection**: Choose to update your **Twitch** channel, **Kick** channel, or **Both Platforms** simultaneously.
...
- **Twitch Tags Management**: Add or remove Twitch stream tags directly from the modal.
- **Cross-Platform Metadata Sync**:
  - Displays instant alerts when titles or categories differ between Twitch and Kick (*"Title or category differs between platforms"*).
  - One-click copy tools: **Copy from Kick**, **Copy title from Kick**, **Copy category from Kick**, or **Copy tags from Kick**.
  - One-click sync buttons: **Use Twitch title** or **Use Kick title**, **Use Twitch category** or **Use Kick category**.

### 2. Multiplatform Live Chat

A consolidated, low-latency live chat feed that brings your audience together:

- **Unified Chat Stream**: Read chat messages from Twitch and Kick side-by-side with clear platform badges and chatter badges.
- **Targeted Sending**: Select your chat platform to send messages or announcements directly to Twitch or Kick chat.
...
- **Delete Message**: Remove inappropriate or violating messages from Twitch chat in real time.
...
- **Permissions Helper**: If your account lacks required Twitch moderator scopes, Stream Manager displays an authorization banner (**Authorize Twitch permissions**) to grant them in one click.
...
- **Read Platform Name Toggle**: When multistreaming, automatically announces which platform the message arrived from (e.g., *"On Kick, ..."*).
...
- **Twitch Events**:
  ...
  - Cheers & Bits (`@{user} cheered {bits} bits`)
  - Channel Point reward redemptions (`@{user} redeemed {reward}`)
...
1. Navigate to **Dashboard → Twitch → `<channel>` → Settings → Moderators** (`/dashboard/twitch/<channel-id>/settings`).
