...
description: "Triggers for Senchabot Twitch bot: chat messages and stream events, match types, permissions and moderation audiences, response actions, variables, and cooldowns."
---

# Twitch Bot — Triggers <Badge type="warning" text="UPDATED"/>

Triggers are automated rules you configure from the dashboard (**Dashboard → Twitch → `<channel>` → Triggers**). They can run when a **chat message** matches your rule or when **stream events** occur (stream start, uptime milestones, category changes, or stream end).
...
Triggers that change stream settings (**Update Stream Category**, **Update Stream Tags**, **Update Stream Title, Tags & Category**) are restricted to trusted channel roles: **VIP**, **Moderator**, or **Broadcaster**. Regular viewers and subscribers cannot trigger stream metadata updates.
...
- VIP status does not bypass this moderation check; only channel **moderators** and the **broadcaster** are exempt.
...
| **Timeout user** | Twitch timeout on the triggering user with a configurable duration |
| **Ban user** | Permanently ban the user from the channel |
| **Update Stream Category** | Change the channel's game/category |
...
- Moderation actions require the bot to have moderator status on your Twitch channel.

### Stream metadata actions

Stream metadata triggers let you update your Twitch stream details automatically from chat commands or stream events:

- **Stream Category**: Set the new game or category name (e.g. `Just Chatting`).
- **Stream Tags**: Set comma-separated tags (e.g. `English, Speedrun`).
...
| **Update Stream Tags** | **Command**: `settags` | Who can trigger: **Moderator** | Stream Tags: `English, Speedrun, Ranked` (Updates stream tags via `!settags`) |
| **Update Stream Title, Tags & Category** | **Command**: `game` | Who can trigger: **Moderator** | Stream Category: `Valorant`, Stream Tags: `FPS, Competitive`, Stream Default Title: `Playing Valorant ranked with viewers!` (Typing `!game Road to Radiant` dynamically sets title to "Road to Radiant", category to Valorant, and tags) |
...
For turn-key automated moderation presets (including **ALL CAPS Detection**, **Link Blocker** with subscriber exemptions, and **Timeout Repeat Offenders**), see the dedicated [Moderation](/twitch-bot/moderation) guide.
