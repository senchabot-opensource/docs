---
title: Kick Bot — Moderation
description: "Automated chat moderation for Senchabot Kick bot: ALL CAPS detection, Link Blocker with subscriber exemptions, Timeout Repeat Offenders, and Blocked Words."
---

# Kick Bot — Moderation <Badge type="warning" text="NEW"/>

Auto-moderation protects your stream chat in real-time by automatically enforcing rules against spam and unwanted links. Configure moderation from the dashboard at **Dashboard → Kick → `<channel>` → Moderation** (`/dashboard/kick/<channel-id>/moderation`).
